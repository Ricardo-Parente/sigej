# repositorio/ordem_servico_repo.py
...
def criar_ordem_servico(numero_sequencial, solicitante_id, area_campus_id, tipo_os_id, equipe_id=None, lider_id=None, prioridade=3, data_prevista=None, descricao_problema=None):
    ...
    """
    cur.execute(sql, (numero_sequencial, solicitante_id, area_campus_id, tipo_os_id, equipe_id, lider_id, prioridade, data_prevista, descricao_problema))
    oid = cur.fetchone()[0]
    conn.commit()
    cur.close()
    conn.close()
...
def adicionar_item_os(os_id, produto_variacao_id, quantidade_prevista):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("INSERT INTO item_ordem_servico (os_id, produto_variacao_id, quantidade_prevista) VALUES (%s, %s, %s) RETURNING id", (os_id, produto_variacao_id, quantidade_prevista))
    iid = cur.fetchone()[0]
    conn.commit()
    cur.close()
    conn.close()
...
def listar_os_abertas():
    ...
    sql = """
    SELECT os.id, os.numero_sequencial, os.prioridade, ac.descricao AS area, tos.descricao AS tipo_servico, p.nome AS solicitante, os.data_abertura
...
def registrar_andamento(os_id, funcionario_id, status_anterior_id, status_novo_id, descricao=None, inicio_atendimento=None, fim_atendimento=None):
    ...
    (os_id, status_anterior_id, status_novo_id, funcionario_id, descricao, inicio_atendimento, fim_atendimento)
    VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id;
    """
    cur.execute(sql, (os_id, status_anterior_id, status_novo_id, funcionario_id, descricao, inicio_atendimento, fim_atendimento))
    aid = cur.fetchone()[0]
    # Atualiza status na OS
    cur.execute("UPDATE ordem_servico SET status_id = %s WHERE id = %s", (status_novo_id, os_id))
    conn.commit()
    cur.close()
    conn.close()
    return aid
